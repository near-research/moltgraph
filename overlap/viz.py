...
#empty-warning b {{ color: var(--accent); }}
...
<div id="main">
  ...
  <div id="tooltip"></div>
  <div id="empty-warning"><b>No nodes visible.</b><br>Try adjusting the filters above.</div>
</div>
...
function loadLayer(layerName) {{
  ...
  // Set filter slider maxes from data
  const maxAuthorCount = d3.max(DATA.nodes, d => d.author_count) || 200;
  const maxSharedAgents = d3.max(DATA.edges, d => d.shared_agents) || 50;
  document.getElementById("filter-authors").max = maxAuthorCount;
  document.getElementById("filter-shared").max = maxSharedAgents;
...
function applyFilters() {{
  ...
  // Empty-state warning
  document.getElementById("empty-warning").style.display = visibleNodes.size === 0 ? "block" : "none";
...
// Sidebar click delegation (bound once, not per update)
document.getElementById("top-bridges").addEventListener("click", (e) => {{
  const el = e.target.closest(".top-item");
  if (!el) return;
  const id = el.dataset.id;
  const node = DATA.nodes.find(n => n.id === id);
  if (node) highlightNeighbors({{stopPropagation:()=>{{}}}}, node);
}});
document.getElementById("community-list").addEventListener("click", (e) => {{
  const el = e.target.closest(".community-row");
  if (!el) return;
